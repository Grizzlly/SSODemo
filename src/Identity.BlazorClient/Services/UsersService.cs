...
namespace Company.WebApps.Identity.BlazorClient.Services
{
    ...
    public class UsersService : IUsersService
    {
        ...
        public async Task<PagedList<UserDetailsViewModel>?> GetUsersAsync(GetUsersRequest request)
        {
            ...
            return await httpClient.GetFromJsonAsync<PagedList<UserDetailsViewModel>>(QueryHelpers.AddQueryString("api/users", queryStringParam));
        ...
        {
            var result = await httpClient.PostAsJsonAsync($"api/users/lock/", userDetails.Id);
        ...
        {
            var result = await httpClient.PostAsJsonAsync($"api/users/unlock/", userDetails.Id);
            return await OperationResult.FromResponseAsync(result);
        }

        public async Task<OperationResult> UpdateUserAsync(UserDetailsViewModel userDetails)
        {
            var result = await httpClient.PostAsJsonAsync($"api/users/{userDetails.Id}", userDetails);
            return await OperationResult.FromResponseAsync(result);
        }
